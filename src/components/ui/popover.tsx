...
export const FloatingPanel = forwardRef<HTMLDivElement, FloatingPanelProps>(({ open, anchorRef, children, id, role, ariaLabel, className, matchAnchorWidth = false, align = "start" }, forwardedRef) => {
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const preferredLeft = align === "center" ? anchorRect.left + (anchorRect.width - panelWidth) / 2 : align === "end" ? anchorRect.right - panelWidth : anchorRect.left;
      const left = Math.min(Math.max(8, preferredLeft), Math.max(8, window.innerWidth - panelWidth - 8));
