...
export const DatePicker = forwardRef<HTMLButtonElement, DatePickerProps>(({ id, name, value = "", onChange, placeholder = "Selecione uma data", className, disabled, onBlur, children, panelAlign, ...props }, ref) => {
  ...
  return <>
    <button ref={setRefs} id={id} name={name} type="button" aria-haspopup="dialog" aria-expanded={open} aria-controls={contentId} disabled={disabled} className={cn("flex h-11 w-full cursor-pointer items-center justify-between gap-3 rounded-lg border border-input bg-background px-3 text-left text-base text-foreground shadow-none outline-none transition-colors focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-60 md:text-sm", !value && "text-muted-foreground", className)} onClick={toggleOpen} onBlur={onBlur} {...props}>{children || <><span className="truncate">{value ? formatDateLabel(value) : placeholder}</span><CalendarDays aria-hidden="true" className="size-4 shrink-0 text-slate-500" /></>}</button>
    <FloatingPanel open={open} anchorRef={triggerRef} id={contentId} role="dialog" ariaLabel="Selecionar data" align={panelAlign} className="p-3"><Calendar month={month} selected={value} onMonthChange={setMonth} onSelect={handleSelect} /></FloatingPanel>
  </>;
});
