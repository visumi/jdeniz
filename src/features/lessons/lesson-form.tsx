...
import { CalendarCheck2, Save, Search, UserRound } from "lucide-react";
...
export function LessonForm({ open, onClose, defaultDate, lesson = null }: LessonFormProps) {
  ...
  const formContent = <form id="lesson-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-5" noValidate>
    {lesson ? <div className="flex items-center gap-3 rounded-xl border border-sky-100 bg-sky-50/70 p-3"><StudentAvatar name={lesson.student.name} /><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950">{lesson.student.name}</p><p className="mt-1 text-xs text-slate-600">O aluno não pode ser trocado nesta edição.</p></div></div> : <div className="space-y-2"><Label htmlFor="lesson-student-search">Aluno <span className="text-red-600">*</span></Label>{selectedStudent ? <div className="flex items-center gap-3 rounded-xl border border-sky-100 bg-sky-50/70 p-3"><StudentAvatar name={selectedStudent.name} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-950">{selectedStudent.name}</p><p className="mt-1 text-xs text-slate-600">{selectedStudent.credits} {selectedStudent.credits === 1 ? "crédito disponível" : "créditos disponíveis"}</p></div><button type="button" className="cursor-pointer text-xs font-semibold text-sky-700 underline underline-offset-2 hover:text-sky-900" onClick={() => setStudent(null)}>Trocar</button></div> : <><div className="relative"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-sky-600" /><Input id="lesson-student-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar pelo nome do aluno" className="pl-9" autoFocus /></div>{searchQuery.length < 2 ? <p className="text-xs text-slate-500">Digite pelo menos 2 letras para buscar.</p> : students.isPending ? <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-600">Buscando alunos…</p> : students.isError ? <p role="alert" className="rounded-lg bg-red-50 p-3 text-sm text-red-800">Não foi possível buscar os alunos agora.</p> : students.data?.length === 0 ? <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-600">Nenhum aluno encontrado.</p> : <div className="max-h-48 space-y-2 overflow-y-auto" role="listbox" aria-label="Resultados de alunos">{students.data?.slice(0, 8).map((item) => <button key={item.id} type="button" className="flex min-h-14 w-full cursor-pointer items-center gap-3 rounded-lg border border-sky-100 bg-white p-3 text-left transition-colors duration-200 hover:border-sky-200 hover:bg-sky-50 focus-visible:bg-sky-50" onClick={() => setStudent(item)}><StudentAvatar name={item.name} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-950">{item.name}</span><span className="mt-1 block text-xs text-slate-600">{item.credits} {item.credits === 1 ? "crédito disponível" : "créditos disponíveis"}</span></span><UserRound aria-hidden="true" className="size-4 shrink-0 text-slate-400" /></button>)}</div>}</>}</div>}
    ...
    <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-sky-100 bg-sky-50/50 p-3"><input type="checkbox" className="mt-0.5 size-4 cursor-pointer accent-sky-600" {...form.register("isMakeup")} /><span><span className="block text-sm font-semibold text-slate-950">Aula de reposição</span><span className="mt-1 block text-xs leading-5 text-slate-600">{changingToMakeup ? `Usará 1 crédito de reposição. Saldo atual: ${currentCredits}.` : "Não altera o saldo enquanto permanecer desmarcada."}</span></span></label>
  ...
  const footer: ReactNode = <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="secondary" onClick={onClose} disabled={mutation.isPending}>Cancelar</Button><Button type="submit" form="lesson-form" disabled={mutation.isPending || (!isEditing && !selectedStudent)}><Save className="size-4" />{mutation.isPending ? "Salvando…" : lesson ? "Salvar alterações" : "Agendar aula"}</Button></div>;
