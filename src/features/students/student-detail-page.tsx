...
function StudentStats({ credits, checkIns, absences }: { credits: number; checkIns: number | string; absences: number | string }) {
...
function StudentCheckinsSection({ query }: { query: ReturnType<typeof useStudentLessons> }) {
  ...
  return <div className="space-y-3"><UpcomingLessonsSection lessons={query.data.upcoming} />{query.data.history.length > 0 && <section aria-labelledby="student-lesson-history-title" className="space-y-3"><h2 id="student-lesson-history-title" className="sr-only">Histórico de check-ins</h2><div className="-mt-1 mx-3 space-y-3 sm:mx-6"><div className="divide-y divide-slate-200 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">{historyItems.map((lesson) => <StudentLessonRow key={lesson.id} lesson={lesson} />)}</div><HistoryPagination page={currentPage} pageCount={pageCount} onPageChange={setHistoryPage} /></div></section>}</div>;
}
...
function WorkoutHistory({ workouts, page, pageCount, onPageChange }: { workouts: Workout[]; page: number; pageCount: number; onPageChange: (page: number) => void }) {
  return <div className="-mt-1 mx-3 max-w-3xl space-y-3 sm:mx-6 lg:max-w-none"><div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100 divide-y divide-slate-200">{workouts.map((workout) => <WorkoutHistoryRow key={workout.id} workout={workout} />)}</div>{/* Temporário para testes: manter a paginação visível mesmo com uma única página. */}<HistoryPagination page={page} pageCount={pageCount} onPageChange={onPageChange} /></div>;
}
...
function StatCard({ icon: Icon, label, value, tone }: { icon: LucideIcon; label: string; value: number | string; tone: "amber" | "emerald" | "red" }) {
  const styles = {
...
function StudentDetailSkeleton() {
  return <div className="space-y-5"><Skeleton className="h-6 w-36" /><Card className="overflow-hidden"><div className="p-5 sm:p-6"><div className="flex items-center gap-4"><Skeleton className="size-14 rounded-full" /><div className="flex-1 space-y-2"><Skeleton className="h-3 w-28" /><Skeleton className="h-8 w-44" /><Skeleton className="h-3 w-32" /></div></div></div><CardContent className="grid gap-6 border-t border-sky-100 pt-5 sm:grid-cols-2 sm:p-6">{[1, 2].map((item) => <div key={item} className="flex items-start gap-3"><Skeleton className="size-9 rounded-lg" /><div className="flex-1 space-y-2"><Skeleton className="h-3 w-24" /><Skeleton className="h-4 w-36" /></div></div>)}</CardContent></Card><div className="grid grid-cols-3 gap-3">{[1, 2, 3].map((item) => <div key={item} className="relative min-h-20 overflow-hidden rounded-xl border border-border bg-card p-3 sm:min-h-24 sm:p-4"><Skeleton className="h-3 w-14" /><Skeleton className="mt-1 h-7 w-8" /><Skeleton className="absolute -bottom-3 -right-3 size-16 rounded-tl-2xl" /></div>)}</div><section className="space-y-3 border-t border-sky-100 pt-5"><div className="flex items-center justify-between gap-3"><Skeleton className="h-7 w-24" /><Skeleton className="h-10 w-28 rounded-md" /></div><WorkoutSkeleton /></section></div>;
}
